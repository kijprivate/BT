...
def get_symbols():
    ...
    for x in range(len(var)):
        if("BTCRUB" in var[x].get("symbol")) or ("BNB" in var[x].get("symbol")) or ("EUR" in var[x].get("symbol")) or ("USD" in var[x].get("symbol")) or ("GRINBTC" in var[x].get("symbol")) or ("HOT" in var[x].get("symbol")) or ("NRG" in var[x].get("symbol")):
            continue
...
def getSymbolResponse(response):
    ...
    for pair in response.get("symbols"):
        newArr.append(pair.get("symbol"))
    return newArr

def getPairPriceEndpoint(pair):
    return '{url}/api/v3/depth?symbol={p}&limit=20'.format(url='https://api.binance.com', p = pair)
